...
curr_1_str = input('Enter currency 1 eg INR:')
curr_2_str = input('Enter currency 2 eg INR:')

url='https://api.exchangeratesapi.io/history?start_at={}&end_at={}&symbols={},{}'.format(start_date,end_date,curr_1_str,curr_2_str)
	...
	if lst[i].strip()[1:11]<end_date and lst[i].strip()[1:11]>start_date:
		...
		while True:
			if lst[i].strip()[1:4] == curr_1_str:
				curr1.append(float(lst[i].strip()[6:-1]))
				curr1_di[k] = float(lst[i].strip()[6:-1])
				f1 = True
...
pre = "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"5000\" height=\"720\">"
suf = "</svg>"
